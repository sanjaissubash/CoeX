...
import { PRODUCT_LIFECYCLES, PRODUCT_STATUSES } from "@/lib/product-options"
...
export function EditProductModal({
  ...
  const [lifecycle, setLifecycle] = useState(product.lifecycle || "IDEA")
  const [status, setStatus] = useState(product.status || "ACTIVE")
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const { push } = useToast()

  // whenever product prop changes, sync local state
  React.useEffect(() => {
    setName(product.name || "")
    setDescription(product.description || "")
    setLifecycle(product.lifecycle || "IDEA")
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const resp = await client.patch(`/products/${product.id}`, {
        name: name.trim(),
      ...
      if (resp.data.success) {
        ...
        onClose()
        push({ title: 'Product saved', description: resp.data.data.name || 'Saved' })
      } else {
      ...
      push({ title: 'Save failed', description: 'Could not save product' })
    } finally {
      setLoading(false)
    }
  }
