...
import { ContextBlocksSection } from "@/components/products/sections/context-blocks-section"
import { DecisionsSection } from "@/components/products/sections/decisions-section"
import { NotesSection } from "@/components/products/sections/notes-section"
import { PromptsSection } from "@/components/products/sections/prompts-section"
import { ResearchSection } from "@/components/products/sections/research-section"
import { SessionsSection } from "@/components/products/sections/sessions-section"
import { TasksSection } from "@/components/products/sections/tasks-section"
    ...
    const load = async () => {
      try {
        ...
        const [tasksRes, notesRes, decRes, ctxRes] = await Promise.all([
          client.get(`/products/${productId}/tasks`).catch(() => ({ data: { data: [] } })),
          client.get(`/notes?product_id=${productId}`).catch(() => ({ data: { data: [] } })),
          client.get(`/products/${productId}/decisions`).catch(() => ({ data: { data: [] } })),
          client.get(`/products/${productId}/context-blocks`).catch(() => ({ data: { data: [] } })),
        ])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card">
        ...
        {activeTab === "planning" && (
          <div className="p-6">
            <TasksSection productId={productId} />
          </div>
        )}
        ...
        {activeTab === "knowledge" && (
          <div className="space-y-6 p-6">
            ...
            <ContextBlocksSection productId={productId} />
            <NotesSection productId={productId} />
            <ResearchSection productId={productId} />
            <DecisionsSection productId={productId} />
          </div>
        )}
        ...
        {activeTab === "selling" && (
          <div className="p-6">
            <h3 className="text-lg font-bold">Selling</h3>
            <div className="text-sm text-muted-foreground">Track reusable listing copy, launch prompts, and selling notes in Knowledge and System.</div>
          </div>
        )}
        ...
        {activeTab === "system" && (
          <div className="space-y-6 p-6">
            ...
            <PromptsSection productId={productId} />
            <SessionsSection productId={productId} />
          </div>
        )}
      </div>
    </div>
...
function AssetsPanel({ productId }: { productId: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [resAssets, resFolders] = await Promise.all([
        client.get(`/products/${productId}/assets`).catch(() => ({ data: { data: [] } })),
        client.get(`/products/${productId}/asset-folders`).catch(() => ({ data: { data: [] } })),
      ])
  ...
  const onFile = async (f: File | null) => {
    ...
    try {
      ...
      const res = await client.post(`/products/${productId}/assets`, form, { headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  const onFolder = async (files: FileList | null) => {
    ...
    try {
      ...
      if (currentFolder) form.append('folder_id', String(currentFolder))
      const res = await client.post(`/products/${productId}/assets/folder`, form, { headers: { 'Content-Type': 'multipart/form-data' } })
      if (res.data && res.data.success) {
  ...
  const createFolder = async () => {
    ...
    try {
      const client = apiClient()
      const res = await client.post(`/products/${productId}/asset-folders`, { name: newFolderName, parent_id: null })
      if (res.data && res.data.success) {
        ...
        <div className="text-sm text-muted-foreground">Loading assets…</div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {assets
                  ...
                  .map((a: any) => {
                    const apiBase = (typeof window !== 'undefined' ? localStorage.getItem('productos_api_url') || process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8082/api' : '')
                    const downloadUrl = `${apiBase}/products/${productId}/assets/download/${a.id}`
