...
export default function PlanningPanel({ productId }: { productId: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await client.get(`/products/${productId}/tasks`)
  ...
  const create = async () => {
    ...
    try {
      const client = apiClient()
      const res = await client.post(`/products/${productId}/tasks`, { title })
