...
export function ResearchSection({ projectId }: { projectId: string }) {
  ...
  useEffect(() => { fetchResearch() }, [projectId])

  const fetchResearch = async () => {
    try {
      const client = apiClient()
      const response = await client.get(`/projects/${projectId}/research`)
