...
from backend.models import ContextBlock, Project, ProjectTemplate, Task, Workspace, Family
...
def _ensure_project_folder(slug: str):
    root = Path(current_app.config.get("STORAGE_ROOT", "storage"))
    prod_dir = root / "projects" / slug
...
def _get_or_create_asset_folder(project_id: str, relative_dir: Path):
    parent_id = None
    current_parts = []
    folder = None
    ...
    for part in relative_dir.parts:
        ...
        folder = AssetFolder.query.filter_by(project_id=project_id, path=folder_path).first()
        if not folder:
            folder = AssetFolder(
...
def _apply_template_defaults(project: Project, template: ProjectTemplate | None):
    if not template:
        return
    ...
    for folder in _loads_template_value(template.default_folder_structure):
        ...
        db.session.add(AssetFolder(project_id=project.id, name=name, path=path))
        (Path(_ensure_project_folder(project.id)) / path).mkdir(parents=True, exist_ok=True)


@api_bp.route("/projects/<pid>/assets", methods=["GET"])
def project_assets_list(pid):
    p = Project.query.get_or_404(pid)
    assets = Asset.query.filter_by(project_id=p.id).order_by(Asset.created_at.desc()).all()
    data = []
    for a in assets:
        data.append({
            "id": a.id,
...
@api_bp.route("/projects/<pid>/asset-folders", methods=["GET"])
def project_asset_folders_list(pid):
    p = Project.query.get_or_404(pid)
    folders = AssetFolder.query.filter_by(project_id=p.id).order_by(AssetFolder.path.asc()).all()
    out = []
    for f in folders:
...
@api_bp.route("/projects/<pid>/asset-folders", methods=["POST"])
def project_asset_folders_create(pid):
    p = Project.query.get_or_404(pid)
    ...
    folder = AssetFolder(project_id=p.id, parent_id=parent_id, name=name, path=path)
    db.session.add(folder)
    db.session.commit()

    # create folder on disk
    prod_folder = Path(_ensure_project_folder(str(p.id)))
...
@api_bp.route("/projects/<pid>/assets", methods=["POST"])
def project_assets_upload(pid):
    p = Project.query.get_or_404(pid)
...
@api_bp.route("/projects/<pid>/assets/folder", methods=["POST"])
def project_assets_folder_upload(pid):
    p = Project.query.get_or_404(pid)
    ...
    if folder_id:
        ...
        if selected_folder.project_id != p.id:
            return jsonify({"success": False, "error": "folder_project_mismatch"}), 400
...
@api_bp.route("/projects/<pid>/assets/download/<asset_id>", methods=["GET"])
def project_asset_download(pid, asset_id):
    p = Project.query.get_or_404(pid)
...
def projects_create():
    ...
    # single global workspace: ensure one exists and use it for created projects
    ...
    p = Project(workspace_id=ws.id, family_id=fam.id, name=name, description=payload.get("description"))
    
    # Store cloud provider and initial metadata in ai_metadata JSON field
    cloud_provider = payload.get("cloud_provider", "Custom")
    p.ai_metadata = {"cloud_provider": cloud_provider}
    ...
    # create project folder using the project id (UUID)
    _ensure_project_folder(str(p.id))
    ...
    # Return serialized project metadata with ai_metadata included
    res_data = _serialize(p)
    res_data["ai_metadata"] = p.ai_metadata
    return jsonify({"success": True, "data": res_data}), 201


@api_bp.route("/projects/<pid>/infra-upload", methods=["POST"])
def projects_infra_upload(pid):
    p = Project.query.get_or_404(pid)
    if "file" not in request.files:
    ...
    cloud_provider = "AWS"
    if p.ai_metadata and isinstance(p.ai_metadata, dict):
        cloud_provider = p.ai_metadata.get("cloud_provider", "AWS")

    # Run parser service to extract details
    from backend.services.infra_parser import InfraParserService
    parsed = InfraParserService.parse_file(str(dest), filename, cloud_provider)

    # Clear any existing auto-generated infra blocks for this project
    ContextBlock.query.filter_by(project_id=p.id, block_type="architecture").delete()
    ContextBlock.query.filter_by(project_id=p.id, block_type="improvements").delete()
    db.session.commit()

    # Create Context Blocks
    arch_block = ContextBlock(
        project_id=p.id,
        title=parsed["architecture"]["title"],
        content=parsed["architecture"]["content"],
        block_type="architecture",
        priority=100
    )
    
    imp_block = ContextBlock(
        project_id=p.id,
        title=parsed["improvements"]["title"],
        content=parsed["improvements"]["content"],
        block_type="improvements",
        priority=90
    )

    db.session.add(arch_block)
    db.session.add(imp_block)
    db.session.commit()
    ...
    ActivityService.log_action(
        ...
        entity_type="Project",
        entity_id=p.id,
        details={"title": f"Uploaded and parsed infrastructure file: {filename}"}
    )

    return jsonify({
        "success": True,
        "message": "Infrastructure file uploaded and parsed successfully",
        "data": {
            "architecture": arch_block.to_dict(),
            "improvements": imp_block.to_dict()
        }
    }), 201



@api_bp.route("/projects/<pid>", methods=["GET"])
...
def projects_patch(pid):
    p = Project.query.get_or_404(pid)
    payload = request.get_json() or {}
    for key in ("name", "description", "lifecycle", "status", "health_score", "ai_metadata"):
        if key in payload:
...
def projects_delete(pid):
    ...
    db.session.delete(p)
    db.session.commit()
    return jsonify({"success": True, "message": f"{project_name} deleted"})
